-- =============================================
-- SETUP ALL: Tables + RLS + Storage + Indexes
-- Copy toàn bộ file này vào Supabase SQL Editor và nhấn Run
-- =============================================

-- 1. Tạo bảng marathon_datasets
...
-- 4. RLS Policies cho marathon_datasets
CREATE POLICY "Users can view their own datasets"
  ON marathon_datasets FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own datasets"
  ON marathon_datasets FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own datasets"
  ON marathon_datasets FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete their own datasets"
  ON marathon_datasets FOR DELETE
  USING (auth.uid() = user_id);

-- 5. RLS Policies cho marathon_players
CREATE POLICY "Users can view players in their datasets"
  ON marathon_players FOR SELECT
  USING (
    EXISTS (
...
CREATE POLICY "Users can insert players in their datasets"
  ON marathon_players FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM marathon_datasets
      WHERE marathon_datasets.id = marathon_players.dataset_id
...
CREATE POLICY "Users can update players in their datasets"
  ON marathon_players FOR UPDATE
  USING (
    EXISTS (
      SELECT 1 FROM marathon_datasets
      WHERE marathon_datasets.id = marathon_players.dataset_id
...
CREATE POLICY "Users can delete players in their datasets"
  ON marathon_players FOR DELETE
  USING (
    EXISTS (
      SELECT 1 FROM marathon_datasets
      WHERE marathon_datasets.id = marathon_players.dataset_id
...
-- 7. Storage bucket cho avatars
INSERT INTO storage.buckets (id, name, public)
VALUES ('marathon-avatars', 'marathon-avatars', true)
